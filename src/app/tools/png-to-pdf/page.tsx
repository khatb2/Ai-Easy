...
import { Download, CornerDownLeft, PlusCircle, Trash2, RotateCw, PanelLeft } from 'lucide-react';
...
export default function PngToPdfPage() {
  ...
  const handleRotateImage = (id: number) => {
    ...
    try {
      if (mergeFiles) {
        ...
        for (const imageFile of imageFiles) {
          ...
          let { width: imgWidth, height: imgHeight } = pngImage;
          
          let pageOrientation = orientation;
          if (pageSize === 'fit' && imgWidth > imgHeight) pageOrientation = 'landscape';
          if (pageSize === 'fit' && imgHeight > imgWidth) pageOrientation = 'portrait';
          
          let [pageWidth, pageHeight] = getPageSizeArr(pageSize, imgWidth, imgHeight);
          if (pageOrientation === 'landscape' && pageWidth < pageHeight) [pageWidth, pageHeight] = [pageHeight, pageWidth];
          else if (pageOrientation === 'portrait' && pageHeight < pageWidth) [pageWidth, pageHeight] = [pageHeight, pageWidth];
          ...
          const scaled = pngImage.scaleToFit(effectiveWidth, effectiveHeight);
          
          page.drawImage(pngImage, {
              x: marginValue + (effectiveWidth - scaled.width) / 2,
              y: marginValue + (effectiveHeight - scaled.height) / 2,
              width: scaled.width,
    ...
    return (
      <div className="flex min-h-screen flex-col">
        ...
        <main className="flex flex-1 flex-col items-center justify-center bg-white text-center">
            <h1 className="text-4xl font-bold">{t.pngToPdfPage.title}</h1>
            <p className="mt-2 text-lg text-muted-foreground">{t.pngToPdfPage.subtitle}</p>
            <input type="file" ref={fileInputRef} onChange={handleFileChange} className="hidden" accept="image/png" multiple />
            <Button onClick={triggerFileInput} className="mt-8" style={{ width: '320px', height: '90px', fontSize: '1.25rem' }}>
              {t.pngToPdfPage.selectButton}
            </Button>
        </main>
        <Footer />
      </div>
  ...
  const SidebarContent = () => (
      <div className="flex h-full flex-col gap-4 p-6">
          <h2 className="text-2xl font-bold">{t.pngToPdfPage.optionsTitle}</h2>
          <div className="space-y-4">
              <div>
                  <Label>{t.pngToPdfPage.orientation}</Label>
                  <RadioGroup value={orientation} onValueChange={(value) => setOrientation(value as any)} className="flex gap-4 mt-2">
                      <div className="flex items-center space-x-2"><RadioGroupItem value="portrait" id="portrait" /><Label htmlFor="portrait">{t.pngToPdfPage.portrait}</Label></div>
                      <div className="flex items-center space-x-2"><RadioGroupItem value="landscape" id="landscape" /><Label htmlFor="landscape">{t.pngToPdfPage.landscape}</Label></div>
                  </RadioGroup>
              </div>
              <div>
                <Label>{t.pngToPdfPage.pageSize}</Label>
                <Select value={pageSize} onValueChange={(v) => setPageSize(v as PageSize)}>
                  <SelectTrigger className="w-full mt-2"><SelectValue placeholder="Select page size" /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="fit">{t.pngToPdfPage.fit}</SelectItem>
                    <SelectItem value="a4">{t.pngToPdfPage.a4}</SelectItem>
                    <SelectItem value="us-letter">{t.pngToPdfPage.usLetter}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <div>
                  <Label>{t.pngToPdfPage.margin}</Label>
                  <RadioGroup value={margin} onValueChange={(v) => setMargin(v as Margin)} className="mt-2 space-y-1">
                      <div className="flex items-center space-x-2"><RadioGroupItem value="no_margin" id="no_margin" /><Label htmlFor="no_margin">{t.pngToPdfPage.noMargin}</Label></div>
                      <div className="flex items-center space-x-2"><RadioGroupItem value="small_margin" id="small_margin" /><Label htmlFor="small_margin">{t.pngToPdfPage.smallMargin}</Label></div>
                       <div className="flex items-center space-x-2"><RadioGroupItem value="big_margin" id="big_margin" /><Label htmlFor="big_margin">{t.pngToPdfPage.bigMargin}</Label></div>
                  </RadioGroup>
              </div>
              <div className="flex items-center space-x-2 pt-4">
                  <Checkbox id="merge" checked={mergeFiles} onCheckedChange={(checked) => setMergeFiles(Boolean(checked))} />
                  <Label htmlFor="merge" className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">{t.pngToPdfPage.merge}</Label>
              </div>
          </div>
          <div className="mt-auto flex flex-col gap-3">
            <Button onClick={handleDownload} size="lg" className="gap-2" disabled={isLoading}>
              {isLoading ? t.pngToPdfPage.loading : <><Download /> {t.pngToPdfPage.downloadButton}</>}
            </Button>
            <Button onClick={handleRestart} variant="secondary" className="gap-2"><CornerDownLeft /> {t.pngToPdfPage.restartButton}</Button>
          </div>
      </div>
  );
      ...
      <div className="flex flex-1">
        ...
        <main className="flex-1 p-4 lg:p-8">
            <div className="flex items-center gap-4 pb-4">
                ...
                <h1 className="text-xl font-semibold sm:text-2xl">{t.pngToPdfPage.title}</h1>
            </div>
              ...
              {imageFiles.map((img, index) => (
                <div key={img.id} className="relative group cursor-move" draggable onDragStart={(e) => handleDragStart(e, index)} onDragEnter={(e) => handleDragEnter(e, index)} onDragEnd={handleDragEnd} onDragOver={(e) => e.preventDefault()}>
                  <div className="overflow-hidden rounded-lg border bg-white shadow-md transition-all duration-300 group-hover:shadow-xl group-hover:-translate-y-1">
                     <div className="w-full h-48 flex items-center justify-center p-2">
                        <img src={img.preview} alt="preview" className="max-h-full max-w-full object-contain transition-transform duration-300 ease-in-out" style={{ transform: `rotate(${img.rotation}deg)` }} />
                     </div>
                  </div>
                  <div className="absolute top-2 right-2 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <Button size="icon" variant="destructive" onClick={() => handleRemoveImage(img.id)}><Trash2 size={16} /></Button>
                    <Button size="icon" onClick={() => handleRotateImage(img.id)}><RotateCw size={16} /></Button>
                  </div>
                </div>
              ))}
                <div className="flex items-center justify-center rounded-lg border-2 border-dashed border-gray-300 cursor-pointer hover:bg-gray-100" onClick={triggerFileInput} style={{height: 200}}>
                    <div className="text-center">
                        ...
                        <p className="mt-2 text-sm text-gray-600">{t.pngToPdfPage.addMore}</p>
                    </div>
                </div>
            </div>
            <input type="file" ref={fileInputRef} onChange={handleFileChange} className="hidden" accept="image/png" multiple />
