...
export default function JpgToPdfPage() {
    ...
    try {
      if (mergeFiles) {
        ...
        for (const imageFile of imageFiles) {
          ...
          let { width: imgWidth, height: imgHeight } = jpgImage;
          ...
          const scaled = jpgImage.scaleToFit(effectiveWidth, effectiveHeight);
    ...
    return (
      <div className="flex min-h-screen flex-col">
        ...
        <main className="flex flex-1 flex-col items-center justify-center bg-white text-center">
            <h1 className="text-4xl font-bold">{t.jpgToPdfPage.title}</h1>
            <p className="mt-2 text-lg text-muted-foreground">{t.jpgToPdfPage.subtitle}</p>
            <input type="file" ref={fileInputRef} onChange={handleFileChange} className="hidden" accept="image/jpeg" multiple />
            <Button onClick={triggerFileInput} className="mt-8" style={{ width: '320px', height: '90px', fontSize: '1.25rem' }}>
              {t.jpgToPdfPage.selectButton}
            </Button>
        </main>
        <Footer />
      </div>
  ...
  const SidebarContent = () => (
    <div className="flex h-full flex-col gap-4 p-6">
        <h2 className="text-2xl font-bold">{t.jpgToPdfPage.optionsTitle}</h2>
      ...
      <div className="flex flex-1">
          ...
          <main className="flex-1 p-4 lg:p-8">
              <div className="flex items-center gap-4 pb-4">
                  ...
                  <h1 className="text-xl font-semibold sm:text-2xl">{t.jpgToPdfPage.title}</h1>
              </div>
